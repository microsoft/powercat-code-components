...
export const SemanticSlots: React.FunctionComponent<ISemanticSlotsProps> = (props: ISemanticSlotsProps) => {
  ...
  const trimPaletteSlots = (paletteSlots: IPalette): IPaletteSlots => {
    const trimmedPaletteSlots: IPaletteSlots = {};
    for (const palette in paletteSlots) {
      if (
  ...
  function trimSemanticSlotsOrNames(
    ...
    if (semanticSlots instanceof Array) {
      const trimmedSemanticSlotNames = [];
      for (let i = 0; i < semanticSlots.length; i++) {
        const slotName = semanticSlots[i];
      ...
      const trimmedSemanticSlots: Partial<ISemanticColors> = {};
      for (const semanticColor in semanticSlots) {
  ...
  const fillVariantSlotsList = (variantType: VariantThemeType): JSX.Element[] => {
    if (props.theme) {
      ...
      const noneVariant = getVariant(props.theme, VariantThemeType.None);
       ...
       */
      for (const semanticColor in trimmedSemanticSlots) {
        if (semanticColor) {
          const paletteColorHexStr = trimmedSemanticSlots[semanticColor as keyof ISemanticColors];
          for (const palette in trimmedPaletteSlots) {
      ...
      for (let i = 0; i < slotNames.length; i++) {
        ...
        const currSlotJSX = semanticSlotWidget(currVariantSemanticSlots[slot as keyof ISemanticColors], mapping[slot]);
  ...
  const semanticSlotsNone = props.theme!.semanticColors;
