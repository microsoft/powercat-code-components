...
import { createMergedRef, css, elementContains, findScrollableParent, FocusZoneDirection, FocusZoneTabbableElements, getDocument, getElementIndexPath, getFocusableByIndexPath, getId, getNativeProps, getNextElement, getParent, getPreviousElement, getRTL, getTheme, getWindow, htmlElementProperties, IFocusZone, IFocusZoneProps, initializeComponentRef, isElementFocusSubZone, isElementFocusZone, isElementTabbable, ITheme, KeyCodes, mergeStyles, Point, portalContainsElement, raiseClick, shouldWrapFocus, warnDeprecations } from '@fluentui/react';
...
export class FocusZone extends React.Component<IFocusZoneProps> implements IFocusZone {
  ...
  public render(): React.ReactNode {
    ...
    return (
      <Tag
        ...
        // ISSUE 1 - Ensure root div has zero tab index so only the contents receives focus when using Ctrl-Tab
        tabIndex={0}
      >
        {this.props.children}
      </Tag>
  ...
  private _setActiveElement(element: HTMLElement, forceAlignment?: boolean): void {
    ...
    if (this._activeElement) {
      if (!this._focusAlignment || forceAlignment) {
       ...
       // ISSUE 1: Positive tabindex Instead of this._activeElement.tabIndex = 0; we use this
       this._activeElement.tabIndex = this.props.tabIndex ? this.props.tabIndex : 0;
  ...
  private _updateTabIndexes(element?: HTMLElement): void {
    // ISSUE 1: Positive tabindex - whereever tabindex '0' is used below - it is replaced by useTabIndex
    const useTabIndex = this.props.tabIndex ? this.props.tabIndex.toString() : '0';
    if (
      !this._activeElement &&
      ...
      if (!isElementFocusZone(child)) {
        ...
        if (isElementTabbable(child)) {
          if (this.props.disabled) {
            ...
            if (child.getAttribute(TABINDEX) !==useTabIndex) {
              child.setAttribute(TABINDEX, useTabIndex);
            }
